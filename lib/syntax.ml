...
module Core = struct
  ...
  let alpha_conv ?(is_stdjsonnet = false) (e : expr) =
    let rec aux env e0 =
      ...
      | Function (xs, e) ->
          ...
          with_
            (Function
               ( List.map (fun (x, y) -> (x, Option.map (aux env) y)) xs,
                 aux env e ))
      | Local (xs, e) ->
            ...
            (Object
               {
                 ...
                 fields =
                   List.map
                     (fun (e1, b, h, e2) -> (e1, b, h, aux env e2))
                     fields;
               })
  ...
  let freevars e =
    let rec aux e =
      match e.v with
      | False | Import _ | Importbin _ | Importstr _ | Null | Number _
      | String _ | True ->
          StringSet.empty
      ...
      | Call (e1, xs, ys) ->
          ...
          let acc =
            ys
            |> List.fold_left
                 (fun acc (_, y) -> StringSet.union acc (aux y))
                 acc
          in
          acc
  ...
  let float_let_binds (e : expr) =
    let local (binds, body) =
      match binds with [] -> body.v | _ -> Local (binds, body)
    in
    let rec aux e =
      let with_ v = { e with v } in
      match e.v with
      | ( False | Import _ | Importbin _ | Importstr _ | Null | Number _ | Self
        | String _ | True | Var _ ) as x ->
          ([], with_ x)
      | Array xs ->
          let floating_binds, xs = xs |> List.map aux |> List.split in
          (List.flatten floating_binds, with_ (Array xs))
      | ArrayIndex (e1, e2) ->
          let floating_binds1, e1 = aux e1 in
          let floating_binds2, e2 = aux e2 in
          (floating_binds1 @ floating_binds2, with_ (ArrayIndex (e1, e2)))
      | Binary (e1, op, e2) ->
          let floating_binds1, e1 = aux e1 in
          let floating_binds2, e2 = aux e2 in
          (floating_binds1 @ floating_binds2, with_ (Binary (e1, op, e2)))
      | Call (e1, xs, ys) ->
          let floating_binds1, e1 = aux e1 in
      ...
      | Function (xs, body) ->
          ...
          in
          ( floating_binds,
            with_ (Function (xs, { body with v = local (fixed_binds, body) }))
          )
      | If (e1, e2, e3) ->
          let floating_binds1, e1 = aux e1 in
          let floating_binds2, e2 = aux e2 in
      ...
      | InSuper e ->
          let floating_binds, e = aux e in
          (floating_binds, with_ (InSuper e))
      | Local (binds, body) ->
          let floating_binds, body = aux body in
          let floating_binds =
            binds
      ...
      | Object { binds; assrts; fields } ->
          ...
          ( floating_binds @ floating_binds31,
            with_ (Object { binds = fixed_binds @ binds; assrts; fields }) )
      | ObjectFor (e1, e2, x, e3) ->
          ...
          ([], with_ (ObjectFor (e1, e2, x, e3)))
      | SuperIndex e ->
          let floating_binds, e = aux e in
          (floating_binds, with_ (SuperIndex e))
      | Unary (op, e) ->
          let floating_binds, e = aux e in
          (floating_binds, with_ (Unary (op, e)))
    in
